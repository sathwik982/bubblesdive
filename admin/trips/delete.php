...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET['tripId'])) {
    ...
    if ($result["status"] === "success" && !empty($result["data"])) {
        $trips = $result["data"][0];
        $confirmation_message = "<h4>Are you sure you want to delete '{$trips['tripType']}' ?</h4>";
    } else {
        $error_message = "No records found or invalid ID.";
    }
}

if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['csrf_token'], $_POST['confirm_delete'], $_POST['tripId']) && $_POST['confirm_delete'] === 'yes') {
    $tripId = $_POST['tripId'];
    if (isset($tripId)) {
        $check_sql = "SELECT * FROM `trips` WHERE tripId = ?";
        $check_params = [$tripId];
