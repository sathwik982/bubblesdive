...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET['id'])) {
    ...
    if ($result["status"] === "success" && !empty($result["data"])) {
        ...
        $confirmation_message = "<h4>Are you sure you want to delete '{$terms_conditions['title']}' ?</h4>";
    } else {
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['csrf_token'], $_POST['confirm_delete'], $_POST['id']) && $_POST['confirm_delete'] === 'yes') {
    $id = $_POST['id'];
    // Check if the record to be deleted actually exists
    $check_sql = "SELECT * FROM `terms_conditions` WHERE id = ?";
    $check_params = [$id];
    $check_result = $db->run($check_sql, $check_params);

    if ($check_result["status"] === "success" && !empty($check_result["data"])) {
        $sql = "DELETE FROM `terms_conditions` WHERE id = ?";
        $params = [$id];
        $result = $db->run($sql, $params);
