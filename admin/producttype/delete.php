...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET['productTypeId'])) {
    ...
    if ($result["status"] === "success" && !empty($result["data"])) {
        ...
        $confirmation_message = "<h4>Are you sure you want to delete '{$producttype['productType']}' ?</h4>";
    } else {
        $error_message = "No records found or invalid ID.";
    }
}

if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['csrf_token'], $_POST['confirm_delete'], $_POST['productTypeId']) && $_POST['confirm_delete'] === 'yes') {
    $productTypeId = $_POST['productTypeId'];
    if (isset($productTypeId)) {
        $check_sql = "SELECT * FROM `producttype` WHERE productTypeId = ?";
        $check_params = [$productTypeId];
