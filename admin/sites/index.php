...
$sql = "SELECT *, DATE_FORMAT(createdDate,'%d-%b-%Y') as `cdate` FROM `divingsites` ORDER BY createdDate DESC";
...
<div class="page-container">
    ...
    <div class="main-content">
        <div class="page-headers">
            <h2 class="header-title">Diving Sites</h2>
                    ...
                    <table id="aboutTable" class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th>Picture</th>
                                <th>Status</th>
                                <?php if ($displaySitesEdit || $displaySitesView || $displaySitesDelete) : ?>
                                    <th>Action</th>
                                <?php endif; ?>
                            </tr>
                        </thead>
                        <tbody>
                            <?php $i = 1; ?>
                            <?php foreach ($divingsites as $sites) : ?>
                                <tr>
                                    <td><?= $i++; ?></td>
                                    <td><?= $sites["siteName"] ?></td>
                                    <td>
                                        <?php if (!empty($sites["picture"])) : ?>
                                            <a href="<?= $sites["picture"] ?>" target="_blank" style="text-decoration:none;">Click me</a>
                                        <?php else : ?>
                                            <span class="-">-</span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?php
                                        ...
                                        ?></td>
                                    <td>
                                        <?php if ($displaySitesEdit) : ?>
                                            <a href="edit.php?id=<?= $sites['id'] ?>" class="btn btn-sm btn-success">Edit</a>
                                        <?php endif; ?>
                                        <?php if ($displaySitesView) : ?>
                                            <a href="view.php?id=<?= $sites['id'] ?>" class="btn btn-sm btn-info">View</a>
                                        <?php endif; ?>
                                        <?php if ($displaySitesDelete) : ?>
                                            <a href="delete.php?id=<?= $sites['id'] ?>" class="btn btn-sm btn-danger">Delete</a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
