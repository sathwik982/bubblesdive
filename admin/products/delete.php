...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET['productId'])) {
    ...
    if ($result["status"] === "success" && !empty($result["data"])) {
        ...
        $confirmation_message = "<h4>Are you sure you want to delete '{$products['productType']}' ?</h4>";
    } else {
        $error_message = "No records found or invalid ID.";
    }
}

if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['csrf_token'], $_POST['confirm_delete'], $_POST['productId']) && $_POST['confirm_delete'] === 'yes') {
    $productId = $_POST['productId'];
    $check_sql = "SELECT p.*, pt.productType 
                  FROM `products` p 
                  LEFT JOIN `producttype` pt ON p.productTypeId = pt.productTypeId
                  WHERE p.productId = ?";
    $check_params = [$productId];
