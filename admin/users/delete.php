...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET['userId'])) {
    ...
    if ($result["status"] === "success" && !empty($result["data"])) {
        $users = $result["data"][0];
        $confirmation_message = "<h4>Are you sure you want to delete '{$users['fullName']}'?</h4>";
    } else {
        $error_message = "No records found or invalid ID.";
    }
}

if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['csrf_token'], $_POST['confirm_delete'], $_POST['userId']) && $_POST['confirm_delete'] === 'yes') {
    $userId = $_POST['userId'];
