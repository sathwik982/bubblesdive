...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_check_changes = "SELECT * FROM `testimonials` WHERE id = ?";
    ...
    if ($result_check_changes["status"] == "success" && !empty($result_check_changes["data"])) {
        ...
        if (
            $existing_tc['authorName'] == $authorName &&
            $existing_tc['feedback'] == $feedback &&
            ...
            $sql = "UPDATE `testimonials` SET authorName = ?, feedback = ?, status = ? WHERE id = ?";
            $params = [$authorName, $feedback, $status, $id];
                        ...
                        <div class="card-body">
                            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="authorName" id="authorName" value="<?= $authorName ?? '' ?>" placeholder="Author Name" required>
                                </div>
                                <div class="form-group shadow-textarea">
                                    <label for="feedback">Feedback</label>
                                    <textarea class="form-control z-depth-1" id="feedback" name="feedback" rows="5" placeholder="Write Something here" required><?= $feedback ?? '' ?></textarea>
                                </div>
                                <div class="mb-3">
